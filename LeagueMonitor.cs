...
namespace Stealth_Ward
{
    class LeagueMonitor
    {
        private static string dataDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Stealth Ward");
        ...
        public static string GetLCUPath()
        {
            ...
            if (!IsValidLCUPath(path))
            {
                ...
                if (leaguePath == null)
                {
                    ...
                    MessageBox.Show(
                        "Stealth Ward could not find the League client at " + path + ". Make sure the League client runs, and press OK.",
                        "LCU not found",
            ...
            while (!IsValidLCUPath(path))
            {
                ...
                MessageBox.Show(
                    "Stealth Ward could not find the running League client. Please select the location of 'LeagueClient.exe' manually.",
                    "LCU not found",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Exclamation
                );
